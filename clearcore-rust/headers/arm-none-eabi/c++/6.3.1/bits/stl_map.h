...
_GLIBCXX_BEGIN_NAMESPACE_CONTAINER
  ...
  */
  template <typename _Key, typename _Tp, typename _Compare = std::less<_Key>,
	...
	iterator  = lower_bound(__k);
	// ->first is greater than or equivalent to __k.
	if ( == end() || key_comp()(__k, (*).first))
#if __cplusplus >= 201103L
	   = _M_t._M_emplace_hint_unique(, std::piecewise_construct,
	...
	return (*).second;
      ...
      {
	const_iterator  = lower_bound(__k);
	if ( == end() || key_comp()(__k, (*).first))
	  __throw_out_of_range(__N("map::at"));
	return (*).second;
      }
...
#define __cpp_lib_map_try_emplace 201411
      ...
      template <typename... _Args>
        ...
        try_emplace(const key_type& __k, _Args&&... __args)
        {
          ...
          if ( == end() || key_comp()(__k, (*).first))
            {
               = emplace_hint(, std::piecewise_construct,
      ...
      template <typename... _Args>
                    ...
                    _Args&&... __args)
        {
          ...
          if (__true_hint.second)
             = emplace_hint(iterator(__true_hint.second),
                               std::piecewise_construct,
                               std::forward_as_tuple(__k),
                               std::forward_as_tuple(
                                 std::forward<_Args>(__args)...));
          else
             = iterator(__true_hint.first);
          return ;
        }
