...
_GLIBCXX_BEGIN_NAMESPACE_VERSION
  ...
  template<typename _CharT, bool _Intl>
    struct __use_cache<__moneypunct_cache<_CharT, _Intl> >
    {
      ...
      {
	const size_t  = moneypunct<_CharT, _Intl>::id._M_id();
	...
	return static_cast<
	  const __moneypunct_cache<_CharT, _Intl>*>(__caches[]);
      }
    };
...
_GLIBCXX_BEGIN_NAMESPACE_LDBL_OR_CXX11
  ...
  template<typename _CharT, typename _InIter>
	...
	for (int  = 0;  < 4 && __testvalid; ++)
	  {
	    const part __which = static_cast<part>(__p.field[]);
	    switch (__which)
	      {
		...
		if (__io.flags() & ios_base::showbase || __sign_size > 1
		    ...
		    || ( == 1 && (__mandatory_sign
					 ...
					 == money_base::space)))
		    || ( == 2 && ((static_cast<part>(__p.field[3])
				      == money_base::value)
				     || (__mandatory_sign
	...
	if (__sign_size > 1 && __testvalid)
	  {
	    ...
	    size_type  = 1;
	    for (; __beg != __end &&  < __sign_size
		   && *__beg == __sign[]; ++__beg, (void)++);
...
_GLIBCXX_END_NAMESPACE_LDBL_OR_CXX11
  ...
  template<typename _CharT, typename _InIter>
			  ...
			  const _CharT* __format) const
    {
      ...
      size_t  = 0;
      for (; __beg != __end &&  < __len && !__tmperr; ++)
	{
	  if (__ctype.narrow(__format[], 0) == '%')
	    {
	      // Verify valid formatting code, attempt to extract.
	      char __c = __ctype.narrow(__format[++], 0);
	      int __mem = 0;
	      if (__c == 'E' || __c == 'O')
		__c = __ctype.narrow(__format[++], 0);
	      switch (__c)
		{
		  const char* __cs;
		  _CharT __wcs[10];
  ...
  template<typename _CharT, typename _InIter>
		   ...
		   ios_base& __io, ios_base::iostate& __err) const
    {
      ...
      size_t  = 0;
      int __value = 0;
      for (; __beg != __end &&  < __len; ++__beg, (void)++)
	{
  ...
  template<typename _CharT, typename _InIter>
	...
	{
	  const char_type __c = *__beg;
	  for (size_t  = 0;  < 2 * __indexlen; ++)
	    if (__c == __names[][0]
		|| __c == __ctype.toupper(__names[][0]))
	      __matches[__nmatches++] = ;
	}
      ...
      if (__nmatches)
	{
	  ...
	  __matches_lengths
	    = static_cast<size_t*>(__builtin_alloca(sizeof(size_t)
						    * __nmatches));
	  for (size_t  = 0;  < __nmatches; ++)
	    __matches_lengths[]
	      = __traits_type::length(__names[__matches[]]);
	}
      ...
      for (; __beg != __end; ++__beg, (void)++__pos)
	{
	  ...
	  for (size_t  = 0;  < __nmatches;)
	    {
	      const char_type* __name = __names[__matches[]];
	      if (__pos >= __matches_lengths[])
		++__nskipped, ++;
	      else if (!(__name[__pos] == __c))
		{
		  --__nmatches;
		  __matches[] = __matches[__nmatches];
		  __matches_lengths[] = __matches_lengths[__nmatches];
		}
	      else
		++;
	    }
