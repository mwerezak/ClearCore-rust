...
_GLIBCXX_BEGIN_NAMESPACE_VERSION
  ...
  template<class _UIntType, int __w, int __n, int __m, int __r,
	...
	__detail::_Shift<_UIntType, __w>::__value>(__value);
      ...
      for (int  = 1;  < state_size; ++)
	{
	  _UIntType __x = _M_x[ - 1];
	  __x ^= __x >> (__w - 2);
	  __x *= 1812433253ul;
	  __x += ;
	  _M_x[] = __detail::__mod<_UIntType, 1, 0,
  ...
  template<typename _IntType, _IntType __m, int __s, int __r>
	...
	__lcg(__value);
      ...
      for (int  = 0;  < long_lag; ++)
	_M_x[] = __detail::__mod<_UIntType, 1, 0, modulus>(__lcg());
	...
	for (int  = 0;  < long_lag; ++)
	  {
	    ...
	    _M_x[] = __detail::__mod<_UIntType, 1, 0, modulus>(__tmp);
	  }
	_M_carry = (_M_x[long_lag - 1] == 0) ? 1 : 0;
	_M_p = 0;
      }
	...
	for (int  = 0;  < long_lag; ++)
	  {
	    for (int __j = 0; __j < __n - 1; ++__j)
	      _M_x[][__j] = __detail::__mod<_UInt32Type, 1, 0, 0>(__gen());
	    _M_x[][__n - 1] = __detail::__mod<_UInt32Type, 1, 0,
