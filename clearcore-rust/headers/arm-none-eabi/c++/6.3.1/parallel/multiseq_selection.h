...
namespace __gnu_parallel
{
  ...
  template<typename _RanSeqs, typename _RankType, typename _RankIterator,
                       ...
                       first_type>::value_type>()) // std::less<_Tp>
    {
      ...
      for (_SeqNumber  = 0;  < __m; ++)
        {
          __nn += std::distance(__begin_seqs[].first,
                               __begin_seqs[].second);
          _GLIBCXX_PARALLEL_ASSERT(
            std::distance(__begin_seqs[].first,
                          __begin_seqs[].second) > 0);
        }

      if (__rank == __nn)
        {
          for (_SeqNumber  = 0;  < __m; ++)
            __begin_offsets[] = __begin_seqs[].second; // Very end.
          // Return __m - 1;
          return;
        }
      ...
      for (_SeqNumber  = 0;  < __m; ++)
        {
          __ns[] = std::distance(__begin_seqs[].first,
                                    __begin_seqs[].second);
          __nmax = std::max(__nmax, __ns[]);
        }

      __r = __rd_log2(__nmax) + 1;

      // Pad all lists to this length, at least as long as any ns[],
      ...
      // Invariants:
      // 0 <= __a[] <= __ns[], 0 <= __b[] <= __l

#define __S() (__begin_seqs[].first)

      // Initial partition.
      std::vector<std::pair<_ValueType, _SeqNumber> > __sample;

      for (_SeqNumber  = 0;  < __m; ++)
        if (__n < __ns[])    //__sequence long enough
          __sample.push_back(std::make_pair(__S()[__n], ));
      __gnu_sequential::sort(__sample.begin(), __sample.end(), __lcomp);

      for (_SeqNumber  = 0;  < __m; ++)       //conceptual infinity
        if (__n >= __ns[])   //__sequence too short, conceptual infinity
          __sample.push_back(
            std::make_pair(__S()[0] /*__dummy element*/, ));
      ...
      while (__n > 0)
        {
          ...
          for ( = 0;  < __m; ++)
            {
              _DifferenceType __middle = (__b[] + __a[]) / 2;
              if (__lmax && __middle < __ns[] &&
                  __lcomp(std::make_pair(__S()[__middle], ),
                        std::make_pair(*__lmax, __lmax_seq)))
                __a[] = std::min(__a[] + __n + 1, __ns[]);
              else
                __b[] -= __n + 1;
            }
          ...
          if (__skew > 0)
            {
              ...
              for (_SeqNumber  = 0;  < __m; ++)
                if (__b[] < __ns[])
                  __pq.push(std::make_pair(__S()[__b[]], ));
          ...
          else if (__skew < 0)
            {
              ...
              for (_SeqNumber  = 0;  < __m; ++)
                if (__a[] > 0)
                  __pq.push(std::make_pair(__S()[__a[] - 1], ));
      ...
      // Postconditions:
      // __a[] == __b[] in most cases, except when __a[] has been
              ...
              if (__a[] > 0)
                {
                  if (!__lmax)
                    ...
                    {
                      if (__comp(*__lmax, __S()[__a[] - 1]))      //max
                        __lmax = &(__S()[__a[] - 1]);
                    }
                }
            }
