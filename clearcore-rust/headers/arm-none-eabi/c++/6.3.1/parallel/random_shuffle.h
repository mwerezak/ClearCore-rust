...
namespace __gnu_parallel
{
  ...
  template<typename _RAIter, typename _RandomNumberGenerator>
	...
	__dist[__b] = 0;
      ...
      // First main loop.
      for (_DifferenceType  = 0;  < __length; ++)
	{
...
#     pragma omp barrier
	...
	__temporaries[__t] = __sd->_M_temporaries[__t];
      ...
      for (_DifferenceType  = 0;  < __length; ++)
	{
          _BinIndex __target_bin = __oracles[];
...
#     pragma omp barrier
      ...
      for (_BinIndex __b = __d->_M_bins_begin; __b < __d->__bins_end; ++__b)
	{
      ...
      for (_SequenceIndex  = 0;  < __offset; ++)
	__sd->_M_temporaries[__iam][].~_ValueType();
...
#       pragma omp single
	{
	  ...
	  for (_ThreadIndex  = 0;  < __num_threads; ++)
	    {
	      __starts[] = __start;
	      __start += ( < __split
			  ? (__chunk_length + 1) : __chunk_length);
	      int __j = __pus[]._M_bins_begin = __bin_cursor;

	      // Range of bins for this processor.
	      __bin_cursor += ( < __bin_split
			       ? (__bin_chunk_length + 1)
			       : __bin_chunk_length);
	      __pus[].__bins_end = __bin_cursor;
	      for (; __j < __bin_cursor; ++__j)
		__sd._M_bin_proc[__j] = ;
	      __pus[]._M_num_threads = __num_threads;
	      __pus[]._M_seed = __rng(std::numeric_limits<uint32_t>::max());
	      __pus[]._M_sd = &__sd;
	    }
  ...
  template<typename _RAIter, typename _RandomNumberGenerator>
      ...
      if (__num_bins > 1)
	{
	    ...
	    static_cast<_ValueType*>(::operator new(sizeof(_ValueType) * __n));
          ...
          for (_DifferenceType  = 0;  < __n; ++)
            ::new(&(__target[(__dist0[__oracles[]])++])) 
		_ValueType(*(__begin + ));
