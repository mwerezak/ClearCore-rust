...
namespace __gnu_parallel
{
  ...
  template<typename _RAIter, typename _DifferenceTp>
	...
	::new(&(__sd->_M_samples[__iam * __num_samples + ]))
...
#     pragma omp barrier
      ...
      for (_DifferenceType  = 0;  < __length_local; ++)
	__sd->_M_temporary[__iam][].~_ValueType();
...
#       pragma omp single
	{
      ...
      if (!__exact)
	{
	  for (_DifferenceType  = 0;  < __size; ++)
	    __sd._M_samples[].~_ValueType();
