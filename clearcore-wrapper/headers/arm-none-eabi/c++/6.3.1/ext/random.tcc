...
_GLIBCXX_BEGIN_NAMESPACE_VERSION
  ...
  template<typename _UIntType, size_t __m,
    ...
    seed(_UIntType __seed)
    {
      ...
      for (size_t  = 1;  < _M_nstate32; ++)
	_M_state32[] = (1812433253UL
			   * (_M_state32[ - 1] ^ (_M_state32[ - 1] >> 30))
			   + );
      _M_pos = state_size;
  ...
  template<typename _UIntType, size_t __m,
	...
	for (size_t  = 1, __j = 0; __j < _M_nstate32; ++__j)
	  {
	    __r = _Func1(_M_state32[]
			 ^ _M_state32[( + __mid) % _M_nstate32]
			 ^ _M_state32[( + _M_nstate32 - 1) % _M_nstate32]);
	    _M_state32[( + __mid) % _M_nstate32] += __r;
	    __r += __arr[__j] + ;
	    _M_state32[( + __mid + __lag) % _M_nstate32] += __r;
	    _M_state32[] = __r;
	     = ( + 1) % _M_nstate32;
	  }
	for (size_t __j = 0; __j < _M_nstate32; ++__j)
	  {
	    const size_t  = (__j + 1) % _M_nstate32;
	    __r = _Func2(_M_state32[]
			 + _M_state32[( + __mid) % _M_nstate32]
			 + _M_state32[( + _M_nstate32 - 1) % _M_nstate32]);
	    _M_state32[( + __mid) % _M_nstate32] ^= __r;
	    __r -= ;
  ...
  template<typename _UIntType, size_t __m,
	...
	if (__parity[] != 0)
	  {
	    _M_state32[] ^= 1 << (__builtin_ffs(__parity[]) - 1);
	    return;
	  }
  ...
  template<typename _UIntType, size_t __m,
    ...
    _M_gen_rand(void)
    {
      ...
      size_t ;
      for ( = 0;  < _M_nstate32 - __pos1_32;  += 4)
	{
	  __recursion<__sl1, __sl2, __sr1, __sr2,
		      __msk1, __msk2, __msk3, __msk4>
	    (&_M_state32[], &_M_state32[],
	     &_M_state32[ + __pos1_32], __r1, __r2);
	  __r1 = __r2;
	  __r2 = &_M_state32[];
	}
      ...
      for (;  < _M_nstate32;  += 4)
	{
	  __recursion<__sl1, __sl2, __sr1, __sr2,
		      __msk1, __msk2, __msk3, __msk4>
	    (&_M_state32[], &_M_state32[],
	     &_M_state32[ + __pos1_32 - _M_nstate32], __r1, __r2);
	  __r1 = __r2;
	  __r2 = &_M_state32[];
	}
  ...
  template<typename _UIntType, size_t __m,
	       ...
	       __parity1, __parity2, __parity3, __parity4>& __x)
    {
      ...
      for (size_t  = 0;  < __x._M_nstate32; ++)
	__os << __x._M_state32[] << __space;
      __os << __x._M_pos;
  ...
  template<typename _UIntType>
	...
	if (__param.total_draws() < __param.total_size() / 2)
	  {
	    for (result_type  = 0;  < __param.total_draws(); ++)
	      {
		if (__b * __aurng() < __a)
		  {
		    ++__k;
	  ...
	  {
	    for (result_type  = 0;  < __param.unsuccessful_size(); ++)
	      {
		if (__b * __aurng() < __a)
		  {
		    ++__k;
