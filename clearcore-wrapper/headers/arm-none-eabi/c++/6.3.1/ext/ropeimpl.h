...
_GLIBCXX_BEGIN_NAMESPACE_VERSION
  ...
  template <class _CharT, class _Alloc>
      ...
      while (__detail::_S_concat == __current_node->_M_tag)
	{
	  ...
	  if (int(_S_path_cache_len) == __current_index)
	    {
	      int ;
	      for ( = 0;  < int(_S_path_cache_len) - 1; ++)
		__x._M_path_end[] = __x._M_path_end[+1];
	      --__current_index;
	    }
	  __current_node =
  ...
  template <class _CharT, class _Alloc>
    ...
    _S_balance(_RopeRep* __r)
    {
      ...
      // __forest[]._M_size >= _S_min_len[]
      // __forest[]._M_depth = 
      // References from forest are included in refcount.
      
      for ( = 0;  <= int(__detail::_S_max_rope_depth); ++)
	__forest[] = 0;
      __try
	{
...
#endif
		__result = _S_concat(__forest[], __result);
		__forest[]->_M_unref_nonnil();
  ...
  template <class _CharT, class _Alloc>
    ...
    _S_add_leaf_to_forest(_RopeRep* __r, _RopeRep** __forest)
    {
      ...
      int ;				// forest[0..-1] is empty
      size_t __s = __r->_M_size;
      
      for ( = 0; __s >= _S_min_len[+1]/* not this bucket */; ++)
	{
	  if (0 != __forest[])
	    {
#ifndef __GC
...
#endif
	      __too_tiny = _S_concat_and_set_balanced(__forest[],
						      __too_tiny);
	      __forest[]->_M_unref_nonnil();
	      __forest[] = 0;
	    }
...
#endif
	      __insertee = _S_concat_and_set_balanced(__forest[],
						      __insertee);
	      __forest[]->_M_unref_nonnil();
	      __forest[] = 0;
	    }
	  if ( == int(__detail::_S_max_rope_depth)
	      || __insertee->_M_size < _S_min_len[+1])
	    {
	      __forest[] = __insertee;
  ...
  template <class _CharT, class _Alloc>
    _CharT
    rope<_CharT, _Alloc>::
    _S_fetch(_RopeRep* __r, size_type )
    {
  ...
  template <class _CharT, class _Alloc>
    _CharT*
    rope<_CharT, _Alloc>::
    _S_fetch_ptr(_RopeRep* __r, size_type )
    {
