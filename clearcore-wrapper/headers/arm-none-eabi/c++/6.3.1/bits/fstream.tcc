...
_GLIBCXX_BEGIN_NAMESPACE_VERSION
  ...
  template<typename _CharT, typename _Traits>
      ...
      if (__testin)
	{
	  ...
	  const bool __testeof = traits_type::eq_int_type(, __ret);
	  int_type __tmp;
	  ...
	  // Try to put back  into input sequence in one of three ways.
	  // Order these tests done in is unspecified by the standard.
	  if (!__testeof && traits_type::eq_int_type(, __tmp))
	    __ret = ;
	  else if (__testeof)
	    __ret = traits_type::not_eof();
	  else if (!__testpb)
	    {
	      _M_create_pback();
	      _M_reading = true;
