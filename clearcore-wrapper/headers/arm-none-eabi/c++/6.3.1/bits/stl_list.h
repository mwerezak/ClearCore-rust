  ...
  */
  template<typename _Tp, typename _Alloc = std::allocator<_Tp> >
	...
	if (!__x.empty())
       ...
       *                      insert before.
       *  @param  __x  Source list.
       *  @param    Const_iterator referencing the element to move.
       *
       *  Removes the element in list @a __x referenced by @a  and
       *  inserts it into the current list before @a __position.
       */
      void
      splice(const_iterator __position, list&& __x, const_iterator ) noexcept
#else
      /**
       *  @brief  Insert element from another %list.
       *  @param  __position  Iterator referencing the element to insert before.
       *  @param  __x  Source list.
       *  @param    Iterator referencing the element to move.
       *
       *  Removes the element in list @a __x referenced by @a  and
       *  inserts it into the current list before @a __position.
       */
      void
      splice(iterator __position, list& __x, iterator )
#endif
      {
	iterator __j = ._M_const_cast();
	++__j;
	if (__position ==  || __position == __j)
	  return;
