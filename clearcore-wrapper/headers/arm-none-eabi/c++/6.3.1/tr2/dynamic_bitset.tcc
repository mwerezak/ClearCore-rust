...
_GLIBCXX_BEGIN_NAMESPACE_VERSION
  ...
  template<typename _WordT, typename _Alloc>
    ...
    __dynamic_bitset_base<_WordT, _Alloc>::_M_do_to_ulong() const
    {
      ...
      for (size_t  = __n;  < this->_M_w.size(); ++)
	if (this->_M_w[])
	  __throw_overflow_error(__N("__dynamic_bitset_base::_M_do_to_ulong"));
      unsigned long __res = 0UL;
      for (size_t  = 0;  < __n &&  < this->_M_w.size(); ++)
	__res += this->_M_w[] << ( * _S_bits_per_block);
      return __res;
    }
  ...
  template<typename _WordT, typename _Alloc>
    ...
    {
      for (size_t  = 0;  < this->_M_w.size(); ++)
	{
	  _WordT __thisword = this->_M_w[];
	  if (__thisword != static_cast<_WordT>(0))
	    return ( * _S_bits_per_block
  ...
  template<typename _WordT, typename _Alloc>
	...
	for (size_t  = __nbits;  > 0; --)
	  {
	    const _CharT __c = __str[__pos + __nbits - ];
