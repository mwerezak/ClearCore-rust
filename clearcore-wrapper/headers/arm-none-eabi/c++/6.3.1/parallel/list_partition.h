...
namespace __gnu_parallel
{
  ...
  template<typename _IIter>
    ...
    {
      for (typename std::vector<_IIter>::size_type  = 0;
            <= (__os_starts.size() / 2); ++)
        __os_starts[] = __os_starts[ * 2];
      __range_length *= 2;
    }
  ...
  template<typename _IIter, typename _FunctorType>
                   ...
                   _FunctorType& __f, int __oversampling = 0)
    {
      ...
      // Smallest partitions.
      for (int  = 1;  < (__num_parts + 1 - __size_greater); ++)
        {
          __lengths[ - 1] =  __size_part * __range_length;
          __index += __size_part;
          __starts[] = __os_starts[__index];
        }

      // Biggest partitions.
      for (int  = __num_parts + 1 - __size_greater;  <= __num_parts;
           ++)
        {
          __lengths[ - 1] =  (__size_part+1) * __range_length;
