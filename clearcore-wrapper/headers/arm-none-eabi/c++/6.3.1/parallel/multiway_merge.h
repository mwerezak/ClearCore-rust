...
#undef _GLIBCXX_PARALLEL_DECISION
  ...
  template<bool __stable,
	...
	for (_SeqNumber __seq = 0; __seq < __k; ++__seq)
      ...
      for (_SeqNumber __s = 0; __s < __k; ++__s)
	for (_DifferenceType  = 0;  < __num_samples; ++)
	  __samples[__s * __num_samples + ].~_ValueType();
...
#         pragma omp single
   ...
   *   int sequences[10][10];
   *   for (int  = 0;  < 10; ++)
   *     for (int __j = 0;  < 10; ++__j)
   *       sequences[][__j] = __j;
   *
   *   int __out[33];
   *   std::vector<std::pair<int*> > seqs;
   *   for (int  = 0;  < 10; ++)
   *     { seqs.push(std::make_pair<int*>(sequences[],
   *                                      sequences[] + 10)) }
   *
   ...
   *   int sequences[10][11];
   *   for (int  = 0;  < 10; ++)
   *     for (int __j = 0;  < 11; ++__j)
   *       sequences[][__j] = __j; // __last one is sentinel!
   *
   *   int __out[33];
   *   std::vector<std::pair<int*> > seqs;
   *   for (int  = 0;  < 10; ++)
   ...
   * @pre Target must provide enough space to merge out length elements or
   *    the number of elements in all sequences, whichever is smaller.
   * @pre For each @c , @c __seqs_begin[].second must be the end
