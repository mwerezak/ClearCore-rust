...
namespace __gnu_parallel
{
  ...
  template<typename _Tp, typename _Compare>
	...
	_M_losers = static_cast<_Loser*>(::operator new(2 * _M_k
							* sizeof(_Loser)));
	for (unsigned int  = _M_ik - 1;  < _M_k; ++)
	  _M_losers[ + _M_k]._M_sup = true;
	...
	if (_M_first_insert)
	  {
	    ...
	    for (unsigned int  = 0;  < (2 * _M_k); ++)
	      ::new(&(_M_losers[]._M_key)) _Tp(__key);
  ...
  template<typename _Tp, typename _Compare>
	...
	_M_losers = static_cast<_Loser*>(::operator new(2 * _M_k
        ...
        for (unsigned int  = 0;  < _M_k; ++)
          {
	    ::new(&(_M_losers[]._M_key)) _Tp(__sentinel);
	    _M_losers[]._M_source = -1;
	  }
        for (unsigned int  = _M_k + _M_ik - 1;  < (2 * _M_k); ++)
          {
	    ::new(&(_M_losers[]._M_key)) _Tp(__sentinel);
	    _M_losers[]._M_source = -1;
	  }
