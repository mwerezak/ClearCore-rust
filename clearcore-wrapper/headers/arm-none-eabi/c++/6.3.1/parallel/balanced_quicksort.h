...
#                 pragma omp flush
  ...
  template<typename _RAIter, typename _Compare>
	...
	{
          __tls[]->_M_elements_leftover = &__elements_leftover;
          __tls[]->_M_num_threads = __num_threads;
          __tls[]->_M_global = std::make_pair(__begin, __end);

          // Just in case nothing is left to assign.
          __tls[]->_M_initial = std::make_pair(__end, __end);
	}
      ...
      _Piece __dummy;
      for (_ThreadIndex  = 1;  < __num_threads; ++)
	_GLIBCXX_PARALLEL_ASSERT(
          !__tls[]->_M_leftover_parts.pop_back(__dummy));
#endif
