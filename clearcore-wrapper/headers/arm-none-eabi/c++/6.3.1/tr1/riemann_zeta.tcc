...
# error do not include this header directly, use <cmath> or <tr1/cmath>
  ...
  _GLIBCXX_BEGIN_NAMESPACE_VERSION
    ...
    __riemann_zeta_alt(_Tp __s)
    {
      _Tp __sgn = _Tp(1);
      _Tp __zeta = _Tp(0);
      for (unsigned int  = 1;  < 10000000; ++)
        {
          _Tp __term = __sgn / std::pow(, __s);
    ...
    __riemann_zeta_glob(_Tp __s)
    {
      ...
      for (unsigned int  = 0;  < __maxit; ++)
        {
          bool __punt = false;
          _Tp __sgn = _Tp(1);
          _Tp __term = _Tp(0);
          for (unsigned int __j = 0; __j <= ; ++__j)
            {
#if _GLIBCXX_USE_C99_MATH_TR1
              _Tp __bincoeff =  _GLIBCXX_MATH_NS::lgamma(_Tp(1 + ))
                              - _GLIBCXX_MATH_NS::lgamma(_Tp(1 + __j))
                              - _GLIBCXX_MATH_NS::lgamma(_Tp(1 +  - __j));
#else
              _Tp __bincoeff =  __log_gamma(_Tp(1 + ))
    ...
    __riemann_zeta_product(_Tp __s)
    {
      ...
      _Tp __zeta = _Tp(1);
      for (unsigned int  = 0;  < __num_primes; ++)
        {
          const _Tp __fact = _Tp(1) - std::pow(__prime[], -__s);
          __zeta *= __fact;
