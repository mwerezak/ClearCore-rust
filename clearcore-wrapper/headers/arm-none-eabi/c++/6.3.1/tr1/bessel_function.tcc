...
# error do not include this header directly, use <cmath> or <tr1/cmath>
  ...
  _GLIBCXX_BEGIN_NAMESPACE_VERSION
                ...
                _Tp & __Jnu, _Tp & __Nnu, _Tp & __Jpnu, _Tp & __Npnu)
    {
      ...
      if (__x < __x_min)
        {
          ...
          for ( = 1;  <= __max_iter; ++)
            {
              __ff = ( * __ff + __p + __q) / ( *  - __mu2);
              __c *= __d / _Tp();
              __p /= _Tp() - __mu;
              __q /= _Tp() + __mu;
              const _Tp __del = __c * (__ff + __r * __q);
              __sum += __del; 
              const _Tp __del1 = __c * __p -  * __del;
              __sum1 += __del1;
      ...
      for ( = 1;  <= __nl; ++)
        {
          const _Tp __Nnutemp = (__mu + ) * __xi2 * __Nnu1 - __Nmu;
          __Nmu = __Nnu1;
          __Nnu1 = __Nnutemp;
        }
      __Nnu = __Nmu;
	...
	return __nu == _Tp(0) ? _Tp(1) : _Tp(0);
      ...
      for (unsigned int  = 1;  < __max_iter; ++)
        {
          __term *= __xx4 / (_Tp() * (__nu + _Tp()));
          __Jn += __term;
